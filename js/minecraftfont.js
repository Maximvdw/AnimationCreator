...
function convertMinecraftTextToHTML(text){
	...
	var regexStrikeout = /(&m)(.*?)(&a|&b|&c|&d|&e|&f|&1|&2|&3|&4|&5|&6|&6|&7|&8|&9|$)/g;
	result = regexStrikeout.exec(text);
	while (result){
		text = text.replace(result[1] + result[2],"<span class='minecraft-effect-strikeout'>" + result[2] + "</span>");
		result = regexStrikeout.exec(text);
	}
	
	var regexUnderline = /(&n)(.*?)(&a|&b|&c|&d|&e|&f|&1|&2|&3|&4|&5|&6|&6|&7|&8|&9|$)/g;
	result = regexUnderline.exec(text);
	while (result){
		text = text.replace(result[1] + result[2],"<span class='minecraft-effect-underline'>" + result[2] + "</span>");
		result = regexUnderline.exec(text);
	}
